...
namespace HackCLR.Parsers.PhpParser.Grammar
{
    public static class PhpKeywords
    {
        ...
        private static IEnumerable<string> AllStringConstants =>
            typeof(PhpKeywords).GetTypeInfo().GetFields().Select(f => f.GetValue(null)).OfType<string>();

        private static IEnumerable<string> AllStringProperties =>
            typeof(PhpKeywords).GetTypeInfo().GetProperties().Select(f => f.GetValue(null)).OfType<string>();
