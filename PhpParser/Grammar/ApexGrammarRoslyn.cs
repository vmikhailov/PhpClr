...
{
	public class ApexGrammarRoslyn : ICommentParserProvider
	{
		...
		protected internal virtual Parser<TypeSyntax> SystemType =>
			Keyword(PhpKeywords.Blob).Or(Keyword(PhpKeywords.Boolean)).Or(Keyword(PhpKeywords.Byte))
			                         ...
			                         .Or(Keyword(PhpKeywords.String)).Or(Keyword(PhpKeywords.List)).Or(Keyword(PhpKeywords.Map))
			                         .Or(Keyword(PhpKeywords.Void))
		...
		protected internal virtual Parser<string> Modifier =>
			Keyword(PhpKeywords.Public).Or(Keyword(PhpKeywords.Protected)).Or(Keyword(PhpKeywords.Private))
			                           .Or(Keyword(PhpKeywords.Static)).Or(Keyword(PhpKeywords.Abstract))
			                           .Or(Keyword(PhpKeywords.Final)).Or(Keyword(PhpKeywords.Global))
			                           .Or(Keyword(PhpKeywords.Override)).Or(Keyword(PhpKeywords.Virtual))
		...
		protected internal virtual Parser<ClassDeclarationSyntax> ClassDeclarationBody =>
			...
			from baseType in Parse.IgnoreCase(PhpKeywords.Extends).Token().Then(t => TypeReference).Optional()
			from interfaces in Parse.IgnoreCase(PhpKeywords.Implements).Token()
			                        .Then(t => TypeReference.DelimitedBy(Parse.Char(',').Token())).Optional()
			from skippedComments in CommentParser.AnyComment.Token().Many()
			from openBrace in Parse.Char('{').Token()
